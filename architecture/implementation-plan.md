# DbDataSync — Implementation Plan
...
- Solution layout:
  - `src/DbDataSync.Api` — ASP.NET Core Web API (orchestrator, scheduler, supervisor).
  - `src/DbDataSync.Web` — React + TypeScript SPA (Vite).
  - `src/DbDataSync.Core` — shared domain models (Connection, Replication/Task, Table Mapping, Column
    Mapping, Scheduling config types) used by both API and Task Runner.
  - `src/DbDataSync.Drivers.Abstractions` — `IDriver`, `IChangeReader`, `IStagingProvider`,
    `IChangeWriter`, driver registry.
  - `src/DbDataSync.Drivers.MsSql` — MSSQL driver implementation.
  - `src/DbDataSync.TaskRunner` — console app, per-run process.
  - `src/DbDataSync.State` — central SQLite access library.
...
- Define YAML schemas in `DbDataSync.Core` for: `Connection`, `Replication`/`Task` (scheduling +
...
- Take a package dependency on `ClrKernel.Core.Secrets.SecretStore` (NuGet) in `DbDataSync.Core`. The
  ...
  credential field. `DbDataSync.TaskRunner` resolves the reference back to a value via the same
...
**Goal**: `DbDataSync.State` provides the single, concurrency-safe access point for run/log/watermark
...
- Schema/migrations for `Tasks`, `TaskRuns`, `ChangeWatermarks`, `Logs`, `RunLocks` (per
  ...
  `DbDataSync.State` dependency-light).
...
- Implement `IDriver`, `IChangeReader`, `IStagingProvider`, `IChangeWriter` in
  `DbDataSync.Drivers.Abstractions`, plus the driver registry.
- `DbDataSync.Drivers.MsSql`:
...
- CLI entrypoint: `DbDataSync.TaskRunner --task-config <path> --run-id <guid>`.
- Pipeline wiring: load task config → resolve driver/reader/cache/writer from the registry → execute
  ...
  status.
- Structured logging to `DbDataSync.State`'s `Logs` table (batched per Phase 2), plus console output
  for local debugging.
...
- Project setup: Vite + React + TypeScript, generated/typed API client against `DbDataSync.Api`.
...
- Revisit `detailed-design.md` §8 open questions with real data: does central-SQLite contention show
  ...
  a problem (fallback path is per-task SQLite files behind the same `DbDataSync.State` interface).
...
- **Batch reload** — a full or list/range-segmented reload/backfill of a table, distinct from the
  ...
  **Built** across Phases 8–10: the per-mapping run model and durable work queue (Phase 8), the segment
  type hierarchy, batch-reload reader and the two reconciling writers (Phase 9), and the Backfill
  trigger endpoint, worker segment wiring, standalone reload replications and capability-driven SPA
  (Phase 10). Remaining refinements are listed in each phase doc's "Notes / things to revisit later".
- Additional source/target database engine drivers (Postgres, MySQL, Oracle, etc.).
- Parquet (or other generic) staging provider.
- Alternate `DbDataSync.State` backends beyond SQLite.
...
- ~~**Rename `TaskRuns.StartedAtUtc` to `EnqueuedAtUtc`.**~~ Done in Phase 73: `TaskRuns` now has
  `EnqueuedAtUtc`, `ClaimedAtUtc` and `StartedAtUtc`, each written at the moment its name says. Kept
  here rather than deleted because phase 72's retrospective points at this entry as the debt it left.
