...
| 7 | **160K** — the docs ship with the app, and the web console can read them | split from the embedded-docs plan. Independent of 158K/159K; **must precede 161K and 162K**. Adds `GET /api/about`, the shared `RichMarkdown` renderer and a Docs rail item |
| 8 | **161K** — Notes can opt in to the rich Markdown renderer | **follows 160K** (reuses its renderer and `/api/about`). Off by default, warned about persistently; needs the setting on Admin Config, `setup` and the CLI |
| 9 | **162K** — images embedded, docs audited, links rewritten at pack time for the NuGet listing | **follows 160K**; independent of 161K. Authors docs and README relative, rewrites to a commit-SHA-pinned form only for the NuGet README |
