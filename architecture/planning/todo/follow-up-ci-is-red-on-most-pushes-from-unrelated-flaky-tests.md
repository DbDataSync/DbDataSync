...
| `35885505054` | `1e2b9ad` (an unrelated dialect-rendering cleanup, not from this session) | `dotnet-integration` → `LibraryValidateIntegrationTests.Validate_AgainstARealServer_SucceedsAndLeavesNoScratchTableBehind` — a spawned child process reported `MsSqlStagingTable/MsSqlMerge failed … Table … was not found`, 1s in. Investigated for causation from `1e2b9ad` (it touches `MsSqlIdentityInsert`/dialect rendering near this pipeline) but the actual scratch-table SQL (`LibraryValidationRunner.BuildCreateScratchTableSql`) wasn't in that commit's diff, and `1e2b9ad`'s own unit suite was reported green. **Not chased further** — the very next push (`35889818477`, identical code) passed this exact test clean, and this test has no prior occurrence in this catalogue, so a single non-reproducing failure isn't enough to confirm a cause rather than guess one. Logged here so a recurrence has a first data point instead of starting from zero. |
