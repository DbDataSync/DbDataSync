...
| `35812820281` | the deadlock fix above | `dotnet-windows` → `WindowsServiceEventLogTests.WriteError_ARealEntryIsReadableBackFromTheApplicationLog` — already-catalogued ([Event Log write denial](follow-up-event-log-tests-guard-registration-but-not-the-write.md)), recurring. This occurrence's own diagnostic (`WindowsElevation.IsAdministrator()` reported `true` at the failure) finally narrowed the cause to one of that doc's two candidates: registration succeeding but not yet propagated to the write path, not a privilege gap. **Fixed** — `WriteOrExplain` retries on the same access-denial for up to 5s (condition-polled, not a fixed sleep) before reporting the original failure; see that doc's own new section for the full reasoning and why no synchronous alternative exists. **Confirmed**: run `35814734262` (the very next push) went green on `dotnet-windows`, the first real proof. |
| `35854242228` | docs-only commit | `dotnet-integration` → `Scd2CdcGuaranteedDeliveryIntegrationTests.ADuplicateKeyStartingOrEndingInADelete…` — `ScanUntilPastAsync`'s own `TimeoutException` this time (every scan attempt in its 30s window completed and reported the same `latest`), not the identical-timestamp race its earlier fix targeted. **Fixed (first attempt)** — deadline widened 30s → 90s, diagnostics sharpened (scan-attempt count, `cdc.lsn_time_mapping` row count); see [`follow-up-phase-154-scd2-cdc-timestamp-mapping-race.md`](follow-up-phase-154-scd2-cdc-timestamp-mapping-race.md)'s own new section. |
| `35901453430` | the three-bug-fix push (unrelated: reader defaults, provisioning 500) | `dotnet-integration` → `Scd2CdcGuaranteedDeliveryIntegrationTests.APassWithDuplicateAndSingletonKeys_AppliesEveryKeyCorrectly_WithNoPkViolation` — the 90s widen's own falsification: 777 scan attempts across the full 90s, every one reporting the identical, unmoved `latest`. That many fast attempts finding nothing is a real stall, not a slow-but-eventually one — ruled out "just needs more time" directly. **Fixed (real cause, not another widen)** — `ScanUntilPastAsync` was re-stopping the capture job and re-releasing the log reader (`sp_repldone @reset = 1`) on *every* 100ms retry, hammering the exact mechanism it was waiting on hundreds of times; now stops once, loops only the scan itself, releases once via `finally`. See that doc's own new section for the full reasoning and what's still unproven. |
| `35882123665` | the deadline-widen fix above | `playwright` → `runs-watermarks-refresh.spec.ts` "03 - the run list polls…" — flagged `1 flaky` (failed once, passed on Playwright's own retry). **Not a product cadence bug** — a test-helper bug found via a live local repro (reproduced ~1 run in 15, instrumented to capture the exact sequence): `RefreshCountdown` renders `data-seconds=""` before its query's first fetch resolves, and the spec's own `seconds()` helper read that through `Number('')`, which is `0` in JavaScript, not `NaN` — indistinguishable from "the real countdown just hit zero." **Fixed** — `NaN` for the empty-string case instead of coercing it; confirmed by 20 consecutive clean local runs and a clean CI run right after (`35889818477`, no flaky annotation). |
| `35885505054` | `1e2b9ad` (an unrelated dialect-rendering cleanup, not from this session) | `dotnet-integration` → `LibraryValidateIntegrationTests.Validate_AgainstARealServer_SucceedsAndLeavesNoScratchTableBehind` — a spawned child process reported `MsSqlStagingTable/MsSqlMerge failed … Table … was not found`, 1s in. **Chased further, 2026-09-24** — not the dialect-rendering commit; the CI log's own timestamps show `DbDataSync.Cli.Tests` (which runs the identical validate-library path via `LibraryValidateCommandTests`, against the same shared `master` database) overlapping this run's own window in the same job, and `LibraryValidationRunner`'s own stale-scratch-table sweep matches on name prefix alone with no notion of age or ownership — one process's sweep could drop the other's still-live table. **Fixed** — see [its own doc](follow-up-library-validate-scratch-table-sweep-races-across-processes.md): the scratch table name now embeds its own creation time, and the sweep only drops one provably older than 5 minutes. |
| `35915250548` | a docs-only commit | `dotnet-integration` → `BulkLoadIntegrationTests.ARaceBetweenAConcurrentReloadAndAMappingsOwnFirstPass_TheLoserFailsCleanly_AndSelfHeals` — `AssertAllSucceeded` got `Failed — Mapping 'map-1' … is still loading` where it expected unconditional success, a different assertion branch than the one [`follow-up-phase-154-bulk-load-race-retrigger-still-observational.md`](follow-up-phase-154-bulk-load-race-retrigger-still-observational.md)'s 2026-09-22 fix already made tolerant of both outcomes. **Not re-diagnosed here** — logged in that doc's own new section; the race this test exercises evidently reaches more than the one call site already patched. |
