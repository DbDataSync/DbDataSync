...
| --- | --- | --- | --- |
| SCD2 CDC identical mapped times | [phase-154 SCD2](follow-up-phase-154-scd2-cdc-timestamp-mapping-race.md) | test-harness; the clock-tick delay (`a2a8d66`) was falsified by a same-day recurrence, replaced with a verified-wait (`ScanUntilPastAsync`) | applied — recurred once more 2026-09-23 (`35854242228`, on an unrelated docs-only commit) as a `TimeoutException` from `ScanUntilPastAsync` itself this time (CDC scan simply hadn't caught up within 30s on the runner, not the identical-timestamp race the original fix targeted), then the very next push (`35857961325`) went green on `dotnet-integration` — still unproven over *consecutive* runs, but no new cause found, just the runner's own CDC latency exceeding the fixed 30s window occasionally |
