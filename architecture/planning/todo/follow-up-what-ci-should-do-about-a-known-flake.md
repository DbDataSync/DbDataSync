...
| --- | --- | --- | --- |
| SCD2 CDC identical mapped times | [phase-154 SCD2](follow-up-phase-154-scd2-cdc-timestamp-mapping-race.md) | test-harness; the clock-tick delay (`a2a8d66`) was falsified by a same-day recurrence, replaced with a verified-wait (`ScanUntilPastAsync`) | applied — recurred once more 2026-09-23 (`35854242228`) as a `TimeoutException` from `ScanUntilPastAsync` itself (every scan attempt in the 30s window completed and reported the same `latest`, so it was the deadline, not the identical-timestamp race the original fix targeted). `ScanUntilPastAsync`'s deadline widened 30s → 90s (the same shape as `c66b834`'s `UpdateConfirmationServiceTests` fix — patience only spent when a run is about to fail), and `DiagnoseAsync` now reports the scan-attempt count and `cdc.lsn_time_mapping`'s row count so the next occurrence, if any, is diagnosable instead of another "still don't know". A candidate third diagnostic (comparing `sys.fn_cdc_get_max_lsn()` against the mapping table to tell "log has more to give" from "CDC has genuinely drained it") was checked live against a real CDC-enabled table before shipping and dropped — that function reads from `cdc.lsn_time_mapping` itself, so the comparison would always report "nothing unmapped" |
...
| `runs-watermarks-refresh.spec.ts` "polls on the ten-second cadence" (new, 2026-09-23, run `35882123665`, `[chromium] 1 flaky`) | — | not a product/cadence bug at all — a test-helper bug: `data-seconds=""` (the DOM's own "not loaded yet" encoding) read through `Number('')`, which is `0` in JavaScript, not `NaN`. That let `toBeLessThanOrEqual(2)` pass on a countdown that had not fetched at all if the read landed in the real (reproduced locally ~1 run in 15) window between the element mounting and its first fetch resolving — no second request ever needed to happen for both polls to report success. Fixed by returning `NaN` for the empty-string case instead of coercing it | applied — root-caused via a live local repro (not guessed): the exact failing sequence was captured with request/response and countdown-value logging, showing zero second `/runs?limit=` request in the network trace while both polls reported success. Confirmed by 20 consecutive clean local runs of the full spec after the fix, unproven yet on CI |
