...
| --- | --- | --- | --- |
| SCD2 CDC identical mapped times | [phase-154 SCD2](follow-up-phase-154-scd2-cdc-timestamp-mapping-race.md) | test-harness; the clock-tick delay (`a2a8d66`) was falsified by a same-day recurrence, replaced with a verified-wait (`ScanUntilPastAsync`) | **both tests disabled 2026-09-23, re-enabled 2026-09-24** — six independent fixes over four days were each falsified by the next occurrence; `[Fact(Skip = ...)]` until 20 consecutive clean solo runs proved a real fix. The seventh fix — `ReleaseLogReaderAsync` moved out of `ScanAsync`/`ScanUntilPastAsync` into each CDC test class's own teardown, closing the release/reacquire gap the four sequential scan calls in the failing test left even after the "release once per call" fix — cleared the disable decision's own bar (20/20 solo runs Ă— 2 tests, 5/5 class runs) and both `[Fact(Skip = ...)]`s were removed. See that doc's own "20-run bar" section |
