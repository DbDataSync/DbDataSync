...
`MsSqlCdc`, `Watermark`, `TriggerAudit`, `PgLogicalSlot`, `OracleFlashback`) do exactly that on a
mapping's first-ever pass: capture the position, stash it, set `ReadHold.Loading`, and hand the actual table read to the
