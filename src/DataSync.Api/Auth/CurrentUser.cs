...
/// <c>GitAuthor("DataSync API", …)</c>, with a comment saying so until per-user auth existed — so the
...
{
    public const string UserIdClaim = "datasync:userId";
    ...
    public GitAuthor Author
    {
        get
        {
            ...
            return new GitAuthor(name, string.IsNullOrWhiteSpace(email) ? "datasync@localhost" : email);
        }
    }
    ...
    public static GitAuthor SystemAuthor { get; } = new("DataSync", "datasync@localhost");
