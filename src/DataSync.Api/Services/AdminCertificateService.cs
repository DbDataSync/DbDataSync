...
/// operation phase 82's <c>datasync cert …</c> already exposes, not a wider one. See that phase's own
/// doc comments (<see cref="DataSync.Certificates"/>) for what each underlying call actually does; this
...
{
    /// <summary>The Windows service name <c>datasync service install</c> registers under — duplicated as
    /// a literal rather than referencing <c>DataSync.Cli.ServiceCommand.ServiceName</c>, the same call
    /// <see cref="CertificateExpiryService"/> already makes: <c>DataSync.Api</c> does not (and should
    /// not) depend on the CLI project.</summary>
    private const string ServiceName = "DataSync";
    ...
    private AdminCertificateStatus GetStatusOnWindows()
    {
        ...
        var config = DataSyncConfigFile.Read(apiOptions.RepoRoot);
        var caConfig = config.GetValueOrDefault("DataSync:Certificates:CaConfig");
    ...
    private CertificateActionResult CreateSelfSignedOnWindows(IReadOnlyList<string> dnsNames, int? validityDays)
    {
        ...
        try
        {
            spec = new CertificateSpec(dnsNames[0], dnsNames, validityDays is > 0 ? validityDays.Value : 397, "DataSync");
        }
    ...
    private CertificateActionResult EnrollOnWindows(IReadOnlyList<string> dnsNames, string template, string? caConfigOverride)
    {
        ...
        var caConfig = string.IsNullOrWhiteSpace(caConfigOverride)
            ? config.GetValueOrDefault("DataSync:Certificates:CaConfig")
            : caConfigOverride;
        if (string.IsNullOrWhiteSpace(caConfig))
        {
            return CertificateActionResult.Failed(
                "No CA is configured. Set DataSync:Certificates:CaConfig in datasync.config.yaml first.");
        }
    ...
    private CertificateActionResult BindOnWindows(string thumbprint, bool? allowInvalidOverride, GitAuthor author)
    {
        ...
        return CertificateActionResult.Ok(
            ...
            "Nothing takes effect until the DataSync service restarts.",
            requestId: null, GetStatusOnWindows());
    }
    ...
    /// DataSync service installed genuinely cannot answer this question, and reporting <c>Ok</c> there
    ...
    internal static KeyAccessInfo EvaluateKeyAccess(X509Certificate2? certificate, string? account)
    {
        if (account is null)
        {
            return new KeyAccessInfo(KeyAccessState.Unknown, null,
                "No DataSync Windows service is installed on this host, so there is no service account to check.");
        }
...
/// (<c>DataSync:Certificates:ExpiryWarningDays</c>) — sent along so the SPA colours "days remaining" by
...
/// <c>datasync cert status</c> reports as an error.</param>
