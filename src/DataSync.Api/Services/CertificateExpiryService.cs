...
/// </para>
/// <para>
/// **Registered only when <see cref="OperatingSystem.IsWindows"/>** (<c>DataSyncHost.Build</c>), the
    ...
    ILogger<CertificateExpiryService> logger) : BackgroundService
{
    ...
    private void CheckOnWindows()
    {
        ...
        if (string.IsNullOrWhiteSpace(subject))
            return; // Nothing bound yet — datasync cert bind hasn't run, or config sets no certificate.
    ...
    private void EvaluateKeyAccess(X509Certificate2 certificate, string subject)
    {
        ...
        if (!PrivateKeyAccess.CanRead(certificate, account))
        {
            logger.LogWarning(
                ...
                "handshake. Run 'datasync cert status' for details, or re-grant access.",
                account, subject, certificate.Thumbprint);
        }
    }

    /// <summary>The Windows service name <c>datasync service install</c> registers under — duplicated
    /// as a literal rather than referencing <c>DataSync.Cli.ServiceCommand.ServiceName</c>, since
    /// <c>DataSync.Api</c> does not (and should not) depend on the CLI project.</summary>
    private const string ServiceCommandName = "DataSync";
}
