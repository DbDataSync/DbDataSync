...
/// would, and so **no certificate password is ever persisted by DataSync**: the private key never
...
/// datasync.config.yaml</c> (the phase 82 doc's own Windows-only test bullet) not actually need to be
...
/// </para>
/// <para>
/// **One section, three literal colons in its name.** <see cref="DataSyncConfigFile.SetValue"/> writes
...
/// cref="DataSyncConfigFile.Read"/>'s flattener turns back into exactly
...
/// <see cref="DataSyncConfigFile.Read"/>.
/// </para>
/// </summary>
public static class CertificateBinding
{
    ...
    public static void Bind(string repoRoot, X509Certificate2 certificate, bool allowInvalid, GitCommitService gitCommitService, GitAuthor author)
    {
        ...
        DataSyncConfigFile.SetValue(repoRoot, Section, "Subject", subject);
        DataSyncConfigFile.SetValue(repoRoot, Section, "Store", "My");
        DataSyncConfigFile.SetValue(repoRoot, Section, "Location", "LocalMachine");
        DataSyncConfigFile.SetValue(repoRoot, Section, "AllowInvalid", allowInvalid ? "true" : "false");

        gitCommitService.CommitChanges(
            [DataSyncConfigFile.PathIn(repoRoot)],
    ...
    {
        var values = DataSyncConfigFile.Read(repoRoot);
