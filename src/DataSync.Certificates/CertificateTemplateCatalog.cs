...
/// <see cref="TemplateListReason.Available"/> — what <c>datasync cert templates</c> prints instead of a
/// list.</param>
...
/// enrollment-services object for the CA named in <c>DataSync:Certificates:CaConfig</c>
    ...
    public static TemplateListResult List(string? caConfig)
    {
        if (string.IsNullOrWhiteSpace(caConfig))
        {
            return TemplateListResult.Empty(
                TemplateListReason.CaConfigNotSet, "DataSync:Certificates:CaConfig is not set.");
        }
