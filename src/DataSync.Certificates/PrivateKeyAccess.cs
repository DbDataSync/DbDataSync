...
/// (which <c>datasync service install --account</c> supports) cannot read it — the symptom is a TLS
...
public static class PrivateKeyAccess
{
    ...
    /// private key — what <c>datasync cert status</c> and the daily expiry check both report, so a
