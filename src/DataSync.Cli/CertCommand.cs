...
/// <summary>
/// <c>datasync cert …</c> — issuance, installation, binding, and renewal of the certificate Kestrel
...
public static class CertCommand
{
    ...
    private static int Status(string[] args)
    {
        ...
        {
            Console.WriteLine(
                "No certificate is bound. Run 'datasync cert new-self-signed' or 'datasync cert enroll', " +
                "then 'datasync cert bind'.");
            return 0;
        }
        ...
        {
            Console.Error.WriteLine(
                $"datasync.config.yaml binds subject '{bound.Subject}' in {location}\\{bound.Store}, but " +
    ...
    private static int NewSelfSigned(string[] args)
    {
        ...
        try
        {
            spec = new CertificateSpec(dnsNames[0], dnsNames, days, FriendlyName: "DataSync");
        }
    ...
    private static int Enroll(string[] args)
    {
        ...
        try
        {
            ...
            spec = new CertificateSpec(dnsNames[0], dnsNames, ValidityDays: 0, FriendlyName: "DataSync");
        }
        catch (ArgumentException ex)
        {
            Console.Error.WriteLine(ex.Message);
    ...
    private static int Renew(string[] args)
    {
        ...
        {
            var spec = new CertificateSpec(subjectCommonName, dnsNames, ValidityDays: 0, FriendlyName: "DataSync");
            return SubmitAndHandle(root, caConfig, template, spec, args);
        }
        ...
        var days = int.TryParse(CliOptions.Read(args, "--days"), out var parsedDays) ? parsedDays : 397;
        var selfSignedSpec = new CertificateSpec(subjectCommonName, dnsNames, days, FriendlyName: "DataSync");
    ...
    private static int Retrieve(string[] args)
    {
        ...
        {
            Console.Error.WriteLine("Usage: datasync cert retrieve --request-id <id>");
            return 1;
        }
        ...
        var spec = new CertificateSpec(pending.SubjectCommonName, pending.DnsNames, ValidityDays: 0, FriendlyName: "DataSync");
    ...
    private static int Templates(string[] args)
    {
        ...
        var caConfig = CliOptions.Read(args, "--ca") ?? config.GetValueOrDefault("DataSync:Certificates:CaConfig");
        ...
        if (result.Reason != TemplateListReason.Available)
        {
            ...
            Console.WriteLine("You can still pass --template with a known template name to 'datasync cert enroll'.");
            return 0;
        }
    ...
    private static int Bind(string[] args)
    {
        ...
        {
            Console.Error.WriteLine(
                "Usage: datasync cert bind --thumbprint <thumbprint> [--location LocalMachine|CurrentUser] " +
        ...
        Console.WriteLine();
        Console.WriteLine("Nothing takes effect until the DataSync service restarts.");
        return 0;
    }
    ...
    private static (string? CaConfig, string? Template) ResolveCaAndTemplate(string root, string[] args, bool requireBoth = true)
    {
        var config = DataSyncConfigFile.Read(root);
        var caConfig = CliOptions.Read(args, "--ca") ?? config.GetValueOrDefault("DataSync:Certificates:CaConfig");
        var template = CliOptions.Read(args, "--template") ?? config.GetValueOrDefault("DataSync:Certificates:Template");
        ...
        if (requireBoth && (string.IsNullOrWhiteSpace(caConfig) || string.IsNullOrWhiteSpace(template)))
        {
            Console.Error.WriteLine(
                ...
                "DataSync:Certificates:CaConfig/Template in datasync.config.yaml.");
            return (null, null);
        }
    ...
    /// <summary>Explicit <c>--account</c> wins outright, else the account the installed DataSync
    ...
    private static void PrintIssued(X509Certificate2 certificate)
    {
        ...
        Console.WriteLine($"  not after   {info.NotAfter:u}");
        Console.WriteLine();
        Console.WriteLine($"  Run 'datasync cert bind --thumbprint {info.Thumbprint}' to put it into service.");
    }
    ...
    private static IReadOnlyList<string>? ReadDnsNames(string[] args)
    {
        ...
        if (string.IsNullOrWhiteSpace(raw))
        {
            Console.Error.WriteLine(
                ...
                "--dns datasync.corp.example.com,datasync");
            return null;
        }
    ...
    private static void PrintUsage() =>
        Console.Error.WriteLine(
            """
            Usage: datasync cert status [--account <account>]
                   datasync cert list [--location LocalMachine|CurrentUser]
                   datasync cert new-self-signed --dns <names> [--days <n>] [--account <account>]
                   datasync cert enroll --dns <names> [--ca <config>] [--template <name>] [--account <account>]
                   datasync cert renew [--ca <config>] [--template <name>] [--account <account>] [--days <n>]
                   datasync cert retrieve --request-id <id> [--account <account>]
                   datasync cert templates [--ca <config>]
                   datasync cert bind --thumbprint <thumbprint> [--location LocalMachine|CurrentUser] [--allow-invalid|--no-allow-invalid]
            """);
}
