...
/// <code>datasync secret set datasync:config:stateConnectionString "Password=..."</code>
/// </summary>
public static class SecretCommand
{
    ...
    private static int Set(string[] args, SecretStore secrets)
    {
        if (args.Length != 3)
        {
            Console.Error.WriteLine("Usage: datasync secret set <ref> <value>");
            return 1;
        }
    ...
    /// (<c>datasync:connection:&lt;name&gt;</c>) checks it without this command having to know
    /// connections exist.
    /// </para>
    /// </summary>
    ...
    private static int Remove(string[] args, SecretStore secrets)
    {
        if (args.Length != 2)
        {
            Console.Error.WriteLine("Usage: datasync secret remove <ref>");
            return 1;
        }
    ...
    private static void PrintUsage() =>
        Console.Error.WriteLine(
            """
            Usage: datasync secret set <ref> <value>
                   datasync secret list [<ref> ...]
                   datasync secret remove <ref>
            """);
}
