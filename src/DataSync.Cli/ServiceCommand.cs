...
/// <para>
/// The binary registered is **the tool's own apphost shim** — the <c>datasync.exe</c> the SDK put in
...
{
    public const string ServiceName = "DataSync";
    ...
    public static int Run(string[] args)
    {
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            Console.Error.WriteLine(
                "Service registration is Windows-only. On Linux, run `datasync serve` under systemd, " +
        ...
        if (args.Length == 0)
        {
            Console.Error.WriteLine("Usage: datasync service install|uninstall|status");
            return 1;
        }
    ...
    private static int Install(string[] args)
    {
        ...
        {
            "create", ServiceName, $"binPath= {binPath}", "start= auto", "DisplayName= DataSync",
        };
