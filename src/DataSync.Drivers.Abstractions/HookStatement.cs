...
/// values interpolated into it. Lives here (rather than in <c>DataSync.Core.Config</c>, beside
/// <c>HookConfig</c>) so <c>DataSync.Scripting.Abstractions</c>'s <c>ILifecycleHook</c> (phase 27) can
/// reference it without that project taking a dependency on <c>DataSync.Drivers.Generic</c>, which is
