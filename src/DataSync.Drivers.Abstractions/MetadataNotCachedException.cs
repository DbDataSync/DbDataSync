...
/// <para>
/// Lives beside the exception it throws rather than in <c>DataSync.Core.Config</c> (where
/// <see cref="CachedColumn"/> itself lives) because the reverse reference already exists —
/// <c>DataSync.Drivers.Abstractions</c> references <c>DataSync.Core</c>, not the other way round (see
