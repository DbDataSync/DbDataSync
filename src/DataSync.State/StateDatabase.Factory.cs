...
public sealed partial class StateDatabase
{
    ...
    /// <see cref="StateDatabase"/> — used by <c>DataSyncHost.cs</c> (the API's own startup) and
    ...
    /// mint a recovery invite. Extracting the branch <c>DataSyncHost.cs</c> already had, rather than
    /// writing a second one, is what makes that fix safe.
    /// </para>
    /// <para>
    /// Lives here rather than in <c>DataSync.Api</c> (where <c>ApiOptions</c> is defined) because
    /// <c>DataSync.Api</c> already depends on <c>DataSync.State</c> for <see cref="StateDatabase"/>
    ...
    /// (via the <c>DataSync.Core</c> project reference), so nothing new is added to
    /// <c>DataSync.State.csproj</c> for this.
    /// </para>
    /// </summary>
        ...
        StateEngine engine, string stateDbPath, string? stateConnectionString, SecretStore secrets)
    {
        ...
        if (stateConnectionString is null)
            throw new InvalidOperationException(
                $"DataSync:StateEngine is '{engine}', which needs DataSync:StateConnectionString. " +
