 ...
 * an operation `datasync cert …` (phase 82) already performs; nothing here can do anything that CLI
 * cannot.
 */
export function AdminCertificatePage() {
  ...
  // Self-signed and enroll only add a certificate to the store — nothing datasync.config.yaml names
      ...
      <div className="pane">
        <div className="page-head">
          ...
          <span className="page-note">
            The TLS certificate DataSync serves, how long it has left, and whether this host can actually
            read its private key. Every action here is a second door onto <code className="mono">datasync
    ...
    <div className="card" data-testid="certificate-binding">
      ...
      <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {(!binding || binding.subject === null) && (
          <span className="faint">datasync.config.yaml names no certificate.</span>
        )}
        {binding && binding.subject !== null && (
          <>
            <span>
              datasync.config.yaml binds subject <span className="mono">{binding.subject}</span> in{' '}
    ...
    <ModalShell title="New self-signed certificate" onCancel={onCancel} testId="self-signed-dialog">
      <Field label="DNS names (comma-separated)">
        <input
          ...
          placeholder="datasync.corp.example.com, datasync"
...
function EnrollDialog({ initialDnsNames, templates, busy, onSubmit, onCancel }: {
  ...
  return (
    <ModalShell title="Enroll from CA" onCancel={onCancel} testId="enroll-dialog">
      ...
      <Field label="CA override (optional — uses DataSync:Certificates:CaConfig otherwise)">
        <input
