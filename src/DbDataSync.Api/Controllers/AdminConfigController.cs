...
/// <summary>
/// The admin config screen (phase 81) — every documented <c>DbDataSync:*</c> key, its effective value and
...
public sealed class AdminConfigController(AdminConfigService service, CurrentUser currentUser) : ControllerBase
{
    ...
    /// <summary>
    /// Writes one key into dbdatasync.config.yaml. The same call for a direct edit of a file-sourced key
    ...
    public ActionResult<AdminConfigEntry> Set(string key, [FromBody] AdminConfigSetRequest body)
    {
        ...
        try
        {
            ...
            return entry is null
                ? NotFound(new { error = $"'{key}' is not a DbDataSync:* key this screen can write." })
                : Ok(entry);
        }
        catch (DbDataSync.Core.Config.ConfigValidationException ex)
        {
    ...
    /// <c>dbdatasync secret set</c> does. The only key this route accepts — see CONFIG.md's "Secrets"
    /// section.
    /// </summary>
    [Authorize(Policies.Admin)]
    [HttpPut("{key}/secret")]
