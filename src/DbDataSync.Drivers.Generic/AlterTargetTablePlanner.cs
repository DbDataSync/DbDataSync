...
/// DbDataSync is not the thing that gets to decide otherwise. The same restraint
        ...
        IReadOnlyList<ColumnMetadata> existing)
    {
        ...
        foreach (var (column, from) in renames)
        {
            steps.Add(new ProvisioningStep(
                ...
                "outside DbDataSync that reads this table by the old name stops working the moment this runs.",
        ...
        foreach (var column in wanted)
        {
            ...
            steps.Add(new ProvisioningStep(
                ...
                "DbDataSync does not know which of the two this is.",
                ProvisioningStepScope.Table));
        }
        ...
        foreach (var (column, from) in renames)
        {
            ...
            if (occupant.Column is null)
            {
                problems.Add(
                    ...
                    "Rename or remove the existing column first — DbDataSync will not overwrite it.");
                continue;
            }
            ...
            var message =
                ...
                "doing both at once needs a temporary name, and DbDataSync does not pick one on your behalf.";
