...
/// <summary>
/// Runs a query an operator wrote, as a full reload, for any engine that can name a
/// <see cref="SqlDialect"/> — the generalization of what began as DuckDB-only
/// (<c>DbDataSync.Drivers.DuckDb.DuckDbQueryReader</c>, still that project's own name for the identical
/// behavior against DuckDB specifically, kept for its existing tests and for the "DuckDbQuery" Kind
/// already saved in mappings on disk).
/// <para>
/// **The query is the configuration.** Every other reader here is handed a table and composes a
/// statement for it; this one is handed a statement and composes nothing. On DuckDB that is the whole
/// point of the driver — the interesting sources are what its scanners reach (<c>read_parquet</c>,
/// <c>read_csv</c>, an attached Postgres, an S3 glob), none of which a table picker can name. On every
/// other engine it is the same escape hatch for the same reason: a view a catalog can't express, a
/// join across tables the mapping model has no notion of, a filter too specific for the generic
/// <c>Filter</c> field to carry cleanly.
/// </para>
/// <para>
...
/// incremental source is <c>ScriptedQueryReader</c>'s job (a compiled query builder can parameterize
/// itself against a stored position; a static operator-typed string cannot).
/// </para>
/// <para>
...
/// </para>
/// </summary>
public class RawQueryReader(string kind, SqlDialect dialect) : IChangeReader, IStatementPreview
{
    ...
    [
        new()
        {
            ...
            Label = "Query",
            Description =
                "The SQL this reader runs, in the source's own dialect. Every row it returns is an " +
                "insert. To segment it, reference the segment's bounds in your own WHERE clause: " +
    ...
    {
        var query = QuerySegmentTokens.Substitute(QueryOf(options), SegmentSerializer.ReadOptional(options), dialect, kind);
        ...
        PreviewRequest request, CancellationToken cancellationToken)
    {
        ...
        var query = QuerySegmentTokens.Substitute(QueryOf(request.Options), segment, dialect, kind);
    ...
    private string QueryOf(IReadOnlyDictionary<string, string> options) =>
        options.TryGetValue(QueryOption, out var query) && !string.IsNullOrWhiteSpace(query)
            ? query
            : throw new InvalidOperationException(
                $"The '{kind}' reader needs a '{QueryOption}' option holding the SQL to run.");
