 ...
 * {@link BulkLoadForm} (opens pre-filled from the mapping's own default segmenting, and the same
 * `.modal-backdrop`/`.modal` popup shell — see that component's own doc comment for why it isn't a
 * fixed-width column beside the live-run panel any more), but with no reader/cache/writer pickers: a
 * sweep always runs KeyReconcile/StagingTable, ending in either KeyReconcileDelete or, since phase 129,
 * KeyReconcileScd2Close when the selected mapping's own writer is Scd2 — there is nothing else this
 * action means, but which of the two endings it means is worth saying (see resolvedReconcileWriterKind
 * below).
 */
...
export function ReconcileDeletesForm({ replicationName, onQueued, onClose }: {
  ...
  return (
    <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <form className="modal" role="dialog" aria-modal="true" aria-label="Reconcile deletes" onSubmit={submit} data-testid="reconcile-deletes-form">
