...
public sealed class AdminCertificateServiceWindowsTests : IDisposable
{
    ...
    {
        var spec = new CertificateSpec(commonName, [commonName], 365, "DataSync Test");
    ...
    {
        DataSyncConfigFile.SetValue(_repoRoot, CertificateBinding.Section, "Subject", subject);
        DataSyncConfigFile.SetValue(_repoRoot, CertificateBinding.Section, "Store", "My");
        DataSyncConfigFile.SetValue(_repoRoot, CertificateBinding.Section, "Location", location);
        DataSyncConfigFile.SetValue(_repoRoot, CertificateBinding.Section, "AllowInvalid", allowInvalid ? "true" : "false");
    }
    ...
    public void GetStatus_NothingBoundYet_ReportsNoCertificate_AndKeyAccessUnknown()
    {
        ...
        // No "DataSync" Windows service is installed in this sandbox (confirmed before writing this
        // test with `sc.exe qc DataSync`) — this is the real, ordinary "the question does not apply"
    ...
    {
        var commonName = $"datasync-test-{Guid.NewGuid():N}.example.com";
    ...
    public void GetStatus_WithAPendingEnrollment_ListsIt_AndRemovingItEmptiesTheListAgain()
    {
        PendingEnrollmentStore.Save(_repoRoot, new PendingEnrollment(
            "req-1", "some-key", "datasync.example.com", ["datasync.example.com"],
        ...
        Assert.Equal("datasync.example.com", pending.SubjectCommonName);
    ...
    {
        var result = Build().Enroll(["datasync.example.com"], "WebServer", caConfig: null);
    ...
    public void Enroll_NoTemplate_Fails() =>
        Assert.False(Build().Enroll(["datasync.example.com"], "", caConfig: "CASERVER\\CA Name").Succeeded);
    ...
    {
        var installed = InstallSelfSigned($"datasync-test-{Guid.NewGuid():N}.example.com");
    ...
    /// <c>DataSyncHost.Build</c> configures for every controller (camelCase, string enums), and checks
