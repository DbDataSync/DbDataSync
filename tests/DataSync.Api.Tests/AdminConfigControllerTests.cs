...
/// <summary>
/// The admin config screen (phase 81): every documented DataSync:* key, its live effective value and
/// source, and — for the keys datasync.config.yaml's writer can address — editing or adopting it.
/// <para>
/// Every test that cares whether a key is "file-sourced" builds its own <see cref="AuthenticatedApiFactory"/>
/// rather than sharing the class fixture: <c>DataSyncHost.InsertConfigFile</c> only wires
/// datasync.config.yaml in as a configuration source if the file already exists the moment the host
...
public sealed class AdminConfigControllerTests(AuthenticatedApiFactory factory) : IClassFixture<AuthenticatedApiFactory>
{
    ...
    [InlineData("PUT", "/api/admin/config/DataSync%3AUrl", true)]
    [InlineData("PUT", "/api/admin/config/DataSync%3AStateConnectionString/secret", true)]
    ...
    public async Task AnAdmin_SeesEveryDocumentedKey()
    {
        ...
        Assert.Contains(entries!, e => e.Key == "DataSync:Auth:Disabled");
        Assert.Contains(entries!, e => e.Key == "DataSync:Auth:Passkeys:Origins");
        ...
        var disabled = entries!.Single(e => e.Key == "DataSync:Auth:Disabled");
        Assert.False(disabled.Editable);
        Assert.False(disabled.CanAdopt);
    }

    /// <summary>
    /// datasync.config.yaml only ever addresses the top-level DataSync:&lt;Key&gt; shape — an Auth:*
    ...
    public async Task ANestedKey_CannotBeWritten()
    {
        ...
        var response = await client.PutAsJsonAsync("/api/admin/config/DataSync%3AAuth%3ADisabled", new { value = "true" });

        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }

    /// <summary>The secret endpoint writes through SecretStore under the fixed ref, exactly like
    /// `datasync secret set` — never through datasync.config.yaml.</summary>
    [Fact]
    public async Task TheSecretEndpoint_StoresThroughSecretStore_NotTheFile()
    {
        ...
        Assert.DoesNotContain(
            "hunter2", DataSyncConfigFile.Read(factory.RepoRoot).GetValueOrDefault(StateConnectionStringKey) ?? "");
    }
    ...
    public async Task AFileSourcedValue_RoundTrips()
    {
        ...
        DataSyncConfigFile.SetValue(scoped.RepoRoot, "DataSync", "Url", "http://initial/");
        ...
        Assert.Equal("http://changed/", DataSyncConfigFile.Read(scoped.RepoRoot)[UrlKey]);
    ...
    {
        using var scoped = new AuthenticatedApiFactory();
        DataSyncConfigFile.SetValue(
            scoped.RepoRoot, "DataSync", "StateConnectionString", "Server=sql01;Database=DataSyncState;");
        var client = await scoped.SignedInAsAsync(UserRole.Admin);
    ...
    public async Task AnEnvVarSourcedValue_IsReadOnly_AndAdoptMovesItIntoTheFile()
    {
        ...
        try
        {
            ...
            Assert.Equal("http://from-env/", DataSyncConfigFile.Read(scoped.RepoRoot)[UrlKey]);
        }
        finally
        {
    ...
    /// operator seeded with a raw Password= the old way, before adopting datasync.config.yaml.
    /// </summary>
    [Fact]
