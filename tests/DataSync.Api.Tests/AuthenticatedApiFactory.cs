...
{
    public string RepoRoot { get; } = Directory.CreateTempSubdirectory("datasync-auth-tests-").FullName;
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.ConfigureAppConfiguration((_, config) =>
        {
            config.AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["DataSync:Auth:AdminGroup"] = "DataSyncAdmins",
                ["DataSync:Auth:ViewerGroup"] = "DataSyncViewers",
            });
        });
