...
public sealed class PasskeyOptionsTests
{
    ...
    {
        RelyingPartyId = id,
        RelyingPartyName = "DataSync",
        Origins = origins.ToHashSet(StringComparer.OrdinalIgnoreCase),
    };

    [Fact]
    public void AMatchingHostAndOrigin_IsFine() =>
        Assert.Null(Options("datasync.corp.example", "https://datasync.corp.example").Problem());
    ...
    public void ASubdomainOrigin_IsUnderTheRelyingParty() =>
        Assert.Null(Options("corp.example", "https://datasync.corp.example").Problem());

    [Fact]
    public void AUrlAsTheId_IsRefusedWithTheReason() =>
        Assert.Contains("bare domain", Options("https://datasync.corp.example").Problem());
    ...
    {
        var problem = Options("datasync.corp.example", "https://datasync.corp.example", "http://localhost:5080")
            .Problem();

        Assert.Contains("http://localhost:5080", problem);
        Assert.Contains("datasync.corp.example", problem);
    }
