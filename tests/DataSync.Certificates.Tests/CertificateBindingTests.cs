...
/// <see cref="DataSyncConfigFile"/>'s real writer and reader, on any OS, with a real (self-signed, since
/// that needs no store) certificate rather than a store-installed one.
/// </summary>
public sealed class CertificateBindingTests : IDisposable
{
    private readonly string _root = Directory.CreateTempSubdirectory("datasync-cert-binding-tests-").FullName;
    ...
    [Fact]
    public void Bind_WritesAllFourKestrelKeys_ReadableBackThroughDataSyncConfigFile()
    {
        ...
        var values = DataSyncConfigFile.Read(_root);
        Assert.Equal("datasync.example.com", values["Kestrel:Certificates:Default:Subject"]);
    ...
    public void Bind_RoundTripsThroughCertificateBindingReadToo()
    {
        ...
        Assert.Equal("datasync.example.com", bound.Subject);
    ...
    public void SubjectCommonName_IsTheSimpleName_NotTheFullDistinguishedName()
    {
        ...
        Assert.Equal("datasync.example.com", CertificateBinding.SubjectCommonName(certificate));
