...
/// <c>datasync service status</c> "already parses" this turned out not to be true, and why this is a
...
public sealed class InstalledServiceAccountTests
{
    private const string RealScQcOutput =
        ...
        "        BINARY_PATH_NAME   : \"C:\\Program Files\\DataSync\\datasync.exe\" serve --repo \"C:\\data\" --url http://localhost:5080\r\n" +
        "        LOAD_ORDER_GROUP   : \r\n" +
        "        TAG                : 0\r\n" +
        "        DISPLAY_NAME       : DataSync\r\n" +
        "        DEPENDENCIES       : \r\n" +
        "        SERVICE_START_NAME : CORP\\svc-datasync\r\n";

    [Fact]
    public void ParsesTheDomainAccount()
    {
        Assert.Equal("CORP\\svc-datasync", InstalledServiceAccount.ParseServiceStartName(RealScQcOutput));
    }

    [Fact]
    public void ParsesLocalSystem()
    {
        var output = RealScQcOutput.Replace("SERVICE_START_NAME : CORP\\svc-datasync", "SERVICE_START_NAME : LocalSystem");
...
/// installed 'DataSync' service (and installing one needs elevation this test should not require), so
...
public sealed class InstalledServiceAccountWindowsTests
{
    ...
    {
        var result = InstalledServiceAccount.Resolve($"DataSync-Does-Not-Exist-{Guid.NewGuid():N}");
