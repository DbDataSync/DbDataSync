...
/// <summary>
/// <c>datasync health</c> falls back to a resolved config's <c>DataSync:Url</c> when <c>--url</c> isn't
/// passed, before falling back to the hardcoded container default — phase 79.
/// </summary>
public sealed class HealthCommandTests : IDisposable
{
    private readonly string _root = Directory.CreateTempSubdirectory("datasync-health-tests-").FullName;
    ...
    public async Task NoUrlFlag_ConfiguredUrlInFile_IsUsed()
    {
        ...
        DataSyncConfigFile.SetValue(_root, "DataSync", "Url", server.Url);
    ...
    {
        using var server = new AnsweringServer();
        DataSyncConfigFile.SetValue(_root, "DataSync", "Url", "http://127.0.0.1:1"); // nothing listens here
