...
/// <summary>
/// <c>ServeCommand.Prepare</c> writes a starter <c>datasync.config.yaml</c> only for a repo root that
/// had no git repository before this run — see the phase 79 doc.
/// </summary>
public sealed class ServeCommandPrepareTests : IDisposable
{
    private readonly string _root = Directory.CreateTempSubdirectory("datasync-serve-prepare-tests-").FullName;
    ...
    public void FreshRoot_GetsAStarterFile_WithTheSecretRefComment()
    {
        ...
        var path = DataSyncConfigFile.PathIn(_root);
        Assert.True(File.Exists(path));
        Assert.Contains("datasync secret set datasync:config:stateConnectionString", File.ReadAllText(path));
    }
    ...
    public void FreshRoot_CommitsTheStarterFile()
    {
        ...
        Assert.NotNull(head);
        Assert.Contains("datasync.config.yaml", head!.Tree.Select(e => e.Path));
    }
    ...
    public void ExistingRepoRoot_SecondPrepareCall_LeavesTheFileUntouched()
    {
        ServeCommand.Prepare(_root);
        var path = DataSyncConfigFile.PathIn(_root);
        File.WriteAllText(path, "DataSync:\n  Url: http://operator-edited/\n");

        ServeCommand.Prepare(_root);

        Assert.Equal("http://operator-edited/", DataSyncConfigFile.Read(_root)["DataSync:Url"]);
    }

    [Fact]
    public void ARepoRootThatAlreadyHadGit_NeverGetsAStarterFile()
    {
        // A pre-existing git repository (not created by Prepare) that simply has no datasync.config.yaml —
        // e.g. an operator who deleted it deliberately to configure entirely by flag/environment variable.
        Repository.Init(_root);

        ServeCommand.Prepare(_root);

        Assert.False(File.Exists(DataSyncConfigFile.PathIn(_root)));
    }
}
