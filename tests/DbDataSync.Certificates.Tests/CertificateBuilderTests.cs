...
public sealed class CertificateBuilderTests
{
    ...
    {
        var spec = new CertificateSpec("dbdatasync.example.com", ["dbdatasync.example.com", "dbdatasync"], 365, null);
        using var certificate = CertificateBuilder.CreateSelfSigned(spec);

        var sanNames = CertificateSanReader.GetDnsNames(certificate);
        Assert.Contains("dbdatasync.example.com", sanNames);
        Assert.Contains("dbdatasync", sanNames);
    }
    ...
    {
        var spec = new CertificateSpec("dbdatasync.example.com", ["dbdatasync.example.com"], 90, null);
        using var certificate = CertificateBuilder.CreateSelfSigned(spec);
    ...
    public void Csr_RoundTripsSubjectAndSans()
    {
        ...
        Assert.Equal("CN=dbdatasync.example.com", loaded.SubjectName.Name);
        ...
        Assert.Contains("dbdatasync.example.com", ExtractDnsNames(sanExtension.RawData));
        Assert.Contains("dbdatasync", ExtractDnsNames(sanExtension.RawData));
    }
