...
public sealed class JdbcReaderParityTests(JdbcTestDatabase db) : IClassFixture<JdbcTestDatabase>, IAsyncLifetime
{
    ...
    public async Task BatchReloadReaderAsync()
    {
        ...
        var npgsqlResult = await npgsqlReader.ReadChangesAsync(
            _npgsql, Source(), null, ReadIntent.InitialLoad, Mappings, "parity", Columns(), [],new Dictionary<string, IReadOnlyList<CachedColumn>>(), options, CancellationToken.None);
        ...
        var jdbcResult = await jdbcReader.ReadChangesAsync(
            _jdbc, Source(), null, ReadIntent.InitialLoad, Mappings, "parity", Columns(), [],new Dictionary<string, IReadOnlyList<CachedColumn>>(), options, CancellationToken.None);
    ...
    public async Task WatermarkReaderAsync()
    {
        ...
        var npgsqlFirst = await npgsqlReader.ReadChangesAsync(
            _npgsql, Source(), null, ReadIntent.Changes, Mappings, "parity", Columns(), [],new Dictionary<string, IReadOnlyList<CachedColumn>>(), options, CancellationToken.None);
        var expectedFirst = await MaterializeAsync(npgsqlFirst.Rows);

        var jdbcFirst = await jdbcReader.ReadChangesAsync(
            _jdbc, Source(), null, ReadIntent.Changes, Mappings, "parity", Columns(), [],new Dictionary<string, IReadOnlyList<CachedColumn>>(), options, CancellationToken.None);
        ...
        var npgsqlSecond = await npgsqlReader.ReadChangesAsync(
            _npgsql, Source(), npgsqlFirst.NewWatermark, ReadIntent.Changes, Mappings, "parity", Columns(), [],new Dictionary<string, IReadOnlyList<CachedColumn>>(), options, CancellationToken.None);
        var expectedSecond = await MaterializeAsync(npgsqlSecond.Rows);

        var jdbcSecond = await jdbcReader.ReadChangesAsync(
            _jdbc, Source(), jdbcFirst.NewWatermark, ReadIntent.Changes, Mappings, "parity", Columns(), [],new Dictionary<string, IReadOnlyList<CachedColumn>>(), options, CancellationToken.None);
