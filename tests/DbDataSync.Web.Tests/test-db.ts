...
export const DB_NAME = 'DbDataSyncPlaywrightTest'
...
export const SA_PASSWORD = 'DbDataSync_Test_Pw1'
...
function sqlcmdArgs(): string[] {
  ...
  try {
    execFileSync('docker', ['exec', 'dbdatasync-mssql-source', 'test', '-x', '/opt/mssql-tools18/bin/sqlcmd'])
...
export function runSql(sql: string, database?: string): void {
  const args = ['exec', 'dbdatasync-mssql-source', ...sqlcmdArgs(), '-S', 'localhost', '-U', 'sa', '-P', SA_PASSWORD]
...
export function querySql(sql: string, database?: string): string {
  const args = ['exec', 'dbdatasync-mssql-source', ...sqlcmdArgs(), '-S', 'localhost', '-U', 'sa', '-P', SA_PASSWORD, '-h', '-1']
  if (database) args.push('-d', database)
  args.push('-Q', sql)
