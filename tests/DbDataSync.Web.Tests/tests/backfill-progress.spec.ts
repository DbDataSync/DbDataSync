 ...
 * like `monitoring-restructure.spec.ts` — what is under test is the card reading `/backfills` and
...
async function stub(page: Page, backfills: unknown[]) {
  ...
  await page.route(`**${base}/backfills`, (route) => route.fulfill(json(backfills)))
...
test.describe('replication detail: batch reload progress (phase 107)', () => {
  test('01 - a running backfill shows rows copied, an estimated total and segment progress', async ({ page }) => {
    await stub(page, [RUNNING_BATCH])
    await page.goto(`/replications/${REPLICATION_NAME}/monitoring`)

    const card = page.getByTestId('backfill-progress-card')
    await expect(card).toBeVisible()
    await expect(card).toHaveAttribute('data-backfill-state', 'Running')

    await expect(page.getByTestId('backfill-rows-copied')).toContainText('240,000')
    await expect(page.getByTestId('backfill-estimated-total')).toContainText('2,400,000')
    await expect(page.getByTestId('backfill-segments')).toContainText('3 / 10')

    // It sits above the reader-lag card it shares the tab with.
    const order = await page
      .locator('[data-testid="backfill-progress-card"], [data-testid="monitoring-range"]')
      .evaluateAll((els) => els.map((el) => el.getAttribute('data-testid')))
    expect(order).toEqual(['backfill-progress-card', 'monitoring-range'])

    await page.screenshot({ path: path.join(screenshotsDir, '110-backfill-progress.png'), fullPage: true })
  })
  ...
  test('02 - the filter caveat shows when the estimate ignored the mapping row filter', async ({ page }) => {
    ...
    await expect(page.getByTestId('backfill-estimate-caveat')).toBeVisible()
  })
  ...
  test('03 - an unknown estimate is not shown as zero', async ({ page }) => {
    ...
    await expect(page.getByTestId('backfill-estimated-total')).toContainText('Unknown')
  })

  test('04 - no card when there is no recent backfill', async ({ page }) => {
    await stub(page, [])
    await page.goto(`/replications/${REPLICATION_NAME}/monitoring`)

    await expect(page.getByTestId('monitoring-range')).toBeVisible()
    await expect(page.getByTestId('backfill-progress-card')).toHaveCount(0)
  })

  test('05 - a long-finished backfill has cleared from the card', async ({ page }) => {
    await stub(page, [{
      ...RUNNING_BATCH,
      state: 'Completed',
