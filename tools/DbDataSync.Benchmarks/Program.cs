...
{
    Console.WriteLine("""
        DbDataSync benchmarks — measures the cost of how a batch of changes is held in memory.
        ...
        Usage: tools/benchmarks [options]
          ...
          --server S      SQL Server connection    (default: the docker-compose source instance,
                                                    or $DBDATASYNC_TEST_MSSQL_SERVER)
...
var scratchDatabase = $"DbDataSyncBenchmark_{Guid.NewGuid():N}";
...
async Task<Measurement> RunChildAsync(Representation representation, string sink, string database)
{
    ...
    startInfo.ArgumentList.Add(Path.Combine(AppContext.BaseDirectory, "DbDataSync.Benchmarks.dll"));
